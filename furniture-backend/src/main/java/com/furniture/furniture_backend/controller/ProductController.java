...
public class ProductController {
        ...
        public ResponseEntity<ApiResponse<PageResponse<ProductResponse>>> searchProductsPaginated(
                        @RequestParam(required = false) String keyword, // Thêm (required = false)
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "10") int size) {

                // Xử lý nếu keyword null thì gán bằng rỗng
                String searchKey = (keyword == null) ? "" : keyword;
                PageResponse<ProductResponse> products = productService.searchProductsPaginated(searchKey, page, size);
        ...
        public ResponseEntity<EntityModel<ProductResponse>> getProductById(@PathVariable Long id) {
                ...
                resource.add(WebMvcLinkBuilder.linkTo(
                                WebMvcLinkBuilder.methodOn(ProductController.class)
                                                .getProductsByCategory(product.getCategoryId()))
        ...
        public ResponseEntity<ApiResponse<List<ProductResponse>>> searchProducts(
                ...
                String searchKey = (keyword == null) ? "" : keyword;
                List<ProductResponse> products = productService.searchProducts(searchKey);
                return ResponseEntity.ok(ApiResponse.success("Search results retrieved successfully", products));
        }
        ...
        public ResponseEntity<ApiResponse<ProductResponse>> createProduct(
                        @Valid @RequestBody ProductRequest request) { // Thêm @Valid
                ProductResponse product = productService.createProduct(request);
                return ResponseEntity.status(HttpStatus.CREATED).body(
                                ApiResponse.success("Thêm sản phẩm thành công", product));
        }
